...
class PollWorker(val context: Context, workerParams: WorkerParameters): Worker(context, workerParams){
    override fun doWork(): Result {
        ...
        val items: List<GalleryItem> = if(query.isEmpty()){
            FlickrRepository().fetchPhotoRequest().execute()
                .body()?.galleryItems
        }else{
            FlickrRepository().searchPhotosRequest(query)
                .execute().body()?.galleryItems
        }?: emptyList()
    ...
    companion object{
        const val ACTION_SHOW_NOTIFICATION = "com.saigyouji.photogallery.SHOW_NOTIFICATION"
        const val PREM_PRIVATE = "com.saigyouji.photogallery.PRIVATE"
